...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('kosan_fasilitas_kamar_mandi', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('fasilitas_kamar_mandi_id');
            $table->timestamps();

            // Foreign keys
            $table->foreign('kosan_id')->references('UniqueID')->on('kosan')->onDelete('cascade');
            $table->foreign('fasilitas_kamar_mandi_id')->references('id')->on('fasilitas_kamar_mandi')->onDelete('cascade');

            // Composite primary key
            $table->primary(['kosan_id', 'fasilitas_kamar_mandi_id']);
        });
    }
