...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('kosan_fasilitas_umum', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('fasilitas_umum_id');
            $table->timestamps();

            // Foreign keys
            $table->foreign('kosan_id')->references('UniqueID')->on('kosan')->onDelete('cascade');
            $table->foreign('fasilitas_umum_id')->references('id')->on('fasilitas_umum')->onDelete('cascade');

            // Composite primary key
            $table->primary(['kosan_id', 'fasilitas_umum_id']);
        });
    }
